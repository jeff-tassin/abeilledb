...
public class BaseTextUI extends TextUI implements ViewFactory, PropertyChangeListener, DocumentListener {
	...
	public void installUI(JComponent c) {
		if (c instanceof JTextComponent) {
			...
			component.setCaretColor(Color.black); // will be changed by settings
													// later
			...
			try {
				...
				if (inputMapClass != null) {
					...
					replaceUIInputMapMethod
							.invoke(null, new Object[] { c, new Integer(JComponent.WHEN_FOCUSED), null });
				}
			} catch (Throwable t) {
			}
	...
	public Dimension getPreferredSize(JComponent c) {
		...
		try {
			...
			d.width = (int) Math.min(rootView.getPreferredSpan(View.X_AXIS) + i.left + i.right + margin.left
					+ margin.right, MAX_SPAN);
			d.height = (int) Math.min(rootView.getPreferredSpan(View.Y_AXIS) + i.top + i.bottom + margin.top
					+ margin.bottom, MAX_SPAN);
		} finally {
			if (doc != null) {
				doc.readUnlock();
			}
	...
	public Dimension getMaximumSize(JComponent c) {
		...
		try {
			d.width = (int) Math.min(rootView.getMaximumSpan(View.X_AXIS) + i.left + i.right + margin.left
					+ margin.right, MAX_SPAN);
			d.height = (int) Math.min(rootView.getMaximumSpan(View.Y_AXIS) + i.top + i.bottom + margin.top
					+ margin.bottom, MAX_SPAN);
		} finally {
			if (doc != null) {
				doc.readUnlock();
