...
public class EditorUI implements ChangeListener, PropertyChangeListener, SettingsChangeListener {
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (settingName == null || SettingsNames.LINE_NUMBER_VISIBLE.equals(settingName)
			...
			lineNumberVisibleSetting = SettingsUtil.getBoolean(kitClass,
					(component != null) ? SettingsNames.LINE_NUMBER_VISIBLE : SettingsNames.PRINT_LINE_NUMBER_VISIBLE,
		...
		if (doc != null) {
			...
			if (settingName == null || SettingsNames.TEXT_LEFT_MARGIN_WIDTH.equals(settingName)) {
				textLeftMarginWidth = SettingsUtil.getInteger(kitClass, SettingsNames.TEXT_LEFT_MARGIN_WIDTH,
			...
			if (settingName == null || SettingsNames.TEXT_LIMIT_LINE_VISIBLE.equals(settingName)) {
				textLimitLineVisible = SettingsUtil.getBoolean(kitClass, SettingsNames.TEXT_LIMIT_LINE_VISIBLE,
						SettingsDefaults.defaultTextLimitLineVisible);
			}
			...
			if (settingName == null || SettingsNames.TEXT_LIMIT_WIDTH.equals(settingName)) {
				textLimitWidth = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.TEXT_LIMIT_WIDTH,
						SettingsDefaults.defaultTextLimitWidth);
			}
					...
					if (fm != null) {
						if (debugUpdateLineHeight) {
							if (maxHeight < fm.getHeight()) {
								System.err.println("Updating maxHeight from " + maxHeight + " to " + fm.getHeight()
										+ ", coloringName=" + coloringName + ", font=" + font);
							}

							if (maxHeight < fm.getHeight()) {
								System.err.println("Updating maxAscent from " + maxAscent + " to " + fm.getAscent()
										+ ", coloringName=" + coloringName + ", font=" + font);
							}
						}
	...
	protected void update(Graphics g) {
		...
		try {
			...
			if (vp != null) {
				java.lang.reflect.Method setScrollModeMethod = JViewport.class.getDeclaredMethod("setScrollMode",
						new Class[] { Integer.TYPE }); // NOI18N
	...
	private Class getKitClass() {
		return (component != null) ? Utilities.getKitClass(component) : ((printDoc != null) ? printDoc.getKitClass()
				: null);
	}
	...
	private boolean scrollRectToVisibleImpl(Rectangle r, int scrollPolicy, Rectangle bounds) {
		...
		if (scrollPolicy == SCROLL_FIND) {
			// converted inset
			int cnvFI = (scrollFindInsets.left < 0) ? (-bounds.width * scrollFindInsets.left / 100)
					: scrollFindInsets.left * defaultSpaceWidth;

			int nx = Math.max(r.x - cnvFI, 0);

			cnvFI = (scrollFindInsets.right < 0) ? (-bounds.width * scrollFindInsets.right / 100)
					: scrollFindInsets.right * defaultSpaceWidth;

			r.width += (r.x - nx) + cnvFI;
			r.x = nx;

			cnvFI = (scrollFindInsets.top < 0) ? (-bounds.height * scrollFindInsets.top / 100) : scrollFindInsets.top
					* lineHeight;

			int ny = Math.max(r.y - cnvFI, 0);

			cnvFI = (scrollFindInsets.bottom < 0) ? (-bounds.height * scrollFindInsets.bottom / 100)
		...
		if (r.width > bounds.width || r.height > bounds.height) {
			...
			if (caretRect.x >= r.x && caretRect.x + caretRect.width <= r.x + r.width && caretRect.y >= r.y
					&& caretRect.y + caretRect.height <= r.y + r.height) { // caret
																			// inside
																			// requested
																			// rect
																			// move
		...
		if (r.x < bounds.x) {
			...
			switch (scrollPolicy) {
			case SCROLL_MOVE:
				newX = (scrollJumpInsets.left < 0) ? (bounds.width * (-scrollJumpInsets.left) / 100)
			...
			case SCROLL_SMALLEST:
				...
				break;
			default:
				newX = (scrollJumpInsets.right < 0) ? (bounds.width * (-scrollJumpInsets.right) / 100)
		...
		if (r.y < bounds.y) {
			...
			case SCROLL_MOVE:
				newY = r.y;
				newY -= (scrollJumpInsets.top < 0) ? (bounds.height * (-scrollJumpInsets.top) / 100)
			...
			case SCROLL_MOVE:
				...
				newY += (scrollJumpInsets.bottom < 0) ? (bounds.height * (-scrollJumpInsets.bottom) / 100)
