...
public class BaseDocument extends AbstractDocument implements SettingsChangeListener {
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (settingName == null || SettingsNames.TAB_SIZE.equals(settingName)) {
			tabSize = SettingsUtil
					.getPositiveInteger(kitClass, SettingsNames.TAB_SIZE, SettingsDefaults.defaultTabSize);
		}
		...
		if (settingName == null || SettingsNames.WRITE_BUFFER_SIZE.equals(settingName)) {
			int writeBufferSize = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.WRITE_BUFFER_SIZE,
		...
		if (settingName == null || SettingsNames.MARK_DISTANCE.equals(settingName)) {
			int markDistance = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.MARK_DISTANCE,
		...
		if (settingName == null || SettingsNames.MAX_MARK_DISTANCE.equals(settingName)) {
			int maxMarkDistance = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.MAX_MARK_DISTANCE,
		...
		if (settingName == null || SettingsNames.MIN_MARK_DISTANCE.equals(settingName)) {
			int minMarkDistance = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.MIN_MARK_DISTANCE,
		...
		if (settingName == null || SettingsNames.READ_MARK_DISTANCE.equals(settingName)) {
			int readMarkDistance = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.READ_MARK_DISTANCE,
		...
		if (settingName == null || SettingsNames.LINE_BATCH_SIZE.equals(settingName)) {
			int lineBatchSize = SettingsUtil.getPositiveInteger(kitClass, SettingsNames.LINE_BATCH_SIZE,
					SettingsDefaults.defaultLineBatchSize);
		...
		if (settingName == null || SettingsNames.IDENTIFIER_ACCEPTOR.equals(settingName)) {
			identifierAcceptor = SettingsUtil.getAcceptor(kitClass, SettingsNames.IDENTIFIER_ACCEPTOR,
					AcceptorFactory.LETTER_DIGIT);
		}

		if (settingName == null || SettingsNames.WHITESPACE_ACCEPTOR.equals(settingName)) {
			whitespaceAcceptor = SettingsUtil.getAcceptor(kitClass, SettingsNames.WHITESPACE_ACCEPTOR,
					AcceptorFactory.WHITESPACE);
		}
		...
		if (settingName == null || SettingsNames.NEXT_WORD_FINDER.equals(settingName)) {
			putProperty(SettingsNames.NEXT_WORD_FINDER, SettingsUtil.getValue(kitClass, SettingsNames.NEXT_WORD_FINDER,
	...
	public void repaintBlock(int startOffset, int endOffset) {
		BaseDocumentEvent evt = createDocumentEvent(startOffset, endOffset - startOffset,
	...
	public void read(Reader reader, int pos) throws IOException, BadLocationException {
		...
		try {
			...
			if (inited || modified) { // was the document already initialized?
				...
				evt.end();
				fireInsertUpdate(evt); // fire the insert event with zero length
	...
	class LineRootElement implements Element {
		...
		public Element getElement(int index) {
			...
			try {
				 ...
				 * (Utilities.getRowCount(BaseDocument.this) - 1) +
				 * " in the document " // NOI18N +
				...
				MarkFactory.LineMark mark = (MarkFactory.LineMark) op.getOffsetMark(startPos,
