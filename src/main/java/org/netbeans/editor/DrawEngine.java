...
class DrawEngine {
		...
		if (ctx.drawMarkUpdate) { // update because of draw mark
			...
			for (int i = 0; i < layersLength; i++) {
				...
				if (l.getName().equals(ctx.drawMark.layerName)
						&& (ctx.drawMark.isDocumentMark() || ctx.editorUI == ctx.drawMark.getEditorUI())) {
	...
	private void computeFragmentDisplayWidth(DrawInfo ctx) {
		...
		if (!ctx.eol) { // handled later
			...
			for (int i = 0; i < layersLength; i++) {
				...
				if (ctx.layerActives[i]) {
					ctx.layers[i].updateContext(ctx); // Let the layer to update
														// the context
				}
			}
		}
		...
		ctx.spaceWidth = (ctx.component != null) ? fmcInfo.getSpaceWidth(ctx.component)
				: ctx.editorUI.defaultSpaceWidth;
		...
		if (ctx.tabsFragment) { // tabs in fragment
			ctx.fragmentCharCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, ctx.fragmentLength,
					ctx.tabSize, ctx.visualColumn) - ctx.visualColumn;
			...
			if (ctx.fragmentLength > 0) {
				if (ctx.component != null) {
					ctx.fragmentWidth = FontMetricsCache.getFontMetrics(ctx.font, ctx.component).charsWidth(
							ctx.slotArray, ctx.fragmentStartIndex, ctx.fragmentLength);
	...
	private void drawFragment(DrawInfo ctx) {
		if (ctx.eol) { // special handling for EOL
				...
				if (ctx.bol) { // empty line found
					if (!emptyLine) { // not yet processed
						...
						if (emptyLine) { // count only if necessary
							blankWidth = ctx.spaceWidth / 2; // display half of
																// char
						}
			...
			if (ctx.tabsFragment) {
				ctx.drawGraphics.drawTabs(ctx.fragmentStartIndex, ctx.fragmentLength, ctx.fragmentCharCount,
						ctx.fragmentWidth, ctx.strikeThroughColor, ctx.underlineColor);
	...
	private void checkTargetOffsetReached(DrawInfo ctx) {
		...
		if (ctx.eol && (ctx.targetOffset == ctx.fragmentOffset || (ctx.targetOffset == -1))) {
			...
			if (ctx.tabsFragment) { // fragment contains tabs
				int spaceCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, high, ctx.tabSize,
						ctx.visualColumn) - ctx.visualColumn;
			...
			if (ctx.tabsFragment) { // fragment contains tabs
				int spaceCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, ctx.fragmentLength,
						ctx.tabSize, ctx.visualColumn) - ctx.visualColumn;
			...
			ctx.continueDraw = ctx.drawGraphics.targetOffsetReached(ctx.fragmentOffset + high,
					ctx.slotArray[ctx.fragmentStartIndex + high], ctx.x + lastWidth, lastWidthP1 - lastWidth, ctx);
			...
			if (!ctx.continueDraw) {
				...
				while (low <= high) {
					...
					if (mid == lastMid + 1) { // try to use cached value
						...
						if (ctx.tabsFragment) { // fragment contains tabs
							int spaceCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, mid,
									ctx.tabSize, ctx.visualColumn) - ctx.visualColumn;
					...
					if (mid == lastMid - 1) { // try to use cached value
						...
						if (ctx.tabsFragment) { // fragment contains tabs
							int spaceCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, mid + 1,
									ctx.tabSize, ctx.visualColumn) - ctx.visualColumn;
					...
					ctx.continueDraw = ctx.drawGraphics.targetOffsetReached(ctx.fragmentOffset + mid,
							ctx.slotArray[ctx.fragmentStartIndex + mid], ctx.x + width, widthP1 - width, ctx);
			...
			if (i > 0) {
				if (ctx.tabsFragment) { // fragment contains tabs
					int spaceCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, i, ctx.tabSize,
							ctx.visualColumn) - ctx.visualColumn;
					prevWidth = spaceCount * ctx.spaceWidth;

				} else { // no tabs inside fragment
					prevWidth = FontMetricsCache.getFontMetrics(ctx.font, ctx.component).charsWidth(ctx.slotArray,
							ctx.fragmentStartIndex, i);
				}
			}

			if (ctx.tabsFragment) { // fragment contains tabs
				int spaceCount = Analyzer.getColumn(ctx.slotArray, ctx.fragmentStartIndex, i + 1, ctx.tabSize,
						ctx.visualColumn) - ctx.visualColumn;
				curWidth = spaceCount * ctx.spaceWidth;

			} else { // no tabs inside fragment
				curWidth = FontMetricsCache.getFontMetrics(ctx.font, ctx.component).charsWidth(ctx.slotArray,
						ctx.fragmentStartIndex, i + 1);
			}

			ctx.continueDraw = ctx.drawGraphics.targetOffsetReached(ctx.fragmentOffset + i,
					ctx.slotArray[ctx.fragmentStartIndex + i], ctx.x + prevWidth, curWidth - prevWidth, ctx);
		}
	}
	...
	private void drawCurrentToken(DrawInfo ctx) {
		...
		if (debug) {
			System.err.println("DrawEngine: TOKEN='" // NOI18N
					+ EditorDebug.debugChars(ctx.getBuffer(), ctx.getTokenOffset() - ctx.getBufferStartOffset(),
							ctx.getTokenLength())
					+ "', tokenID=<"
	...
	private void graphicsSpecificUpdates(DrawInfo ctx) {
		...
		if (ctx.lineNumbering && !ctx.syncedLineNumbering) {
			...
			for (int j = 0; j < ctx.lineIndex; j++) { // draw all line numbers
				...
				ctx.drawGraphics.drawChars(0, ctx.lineNumberChars.length, ctx.lineNumberChars.length
						* ctx.editorUI.lineNumberDigitWidth, ctx.strikeThroughColor, ctx.underlineColor);
		...
		if (ctx.editorUI.textLeftMarginWidth > 0 && bounds.x > 0) {
			ctx.graphics.fillRect(bounds.x + textMargin.left - ctx.editorUI.textLeftMarginWidth, ctx.startY,
					ctx.editorUI.textLeftMarginWidth, ctx.lineIndex * ctx.lineHeight);
		}

		// Right margin
		if (textMargin.right > 0) {
			ctx.graphics.fillRect(bounds.x + bounds.width - textMargin.right, ctx.startY, textMargin.right,
					ctx.lineIndex * ctx.lineHeight);
		}
	 ...
	 *            point.
	 */
	void draw(DrawGraphics drawGraphics, EditorUI editorUI, int startOffset, int endOffset, int startX, int startY,
			int targetOffset) throws BadLocationException {
		...
		synchronized (editorUI) { // lock operations manipulating draw layer
			...
			try {
				...
				ctx.editorUI.updateVirtualWidth(ctx.widestWidth + ctx.editorUI.lineNumberWidth + 2
						* ctx.editorUI.defaultSpaceWidth);
			...
			while (true) {
				if (mark instanceof MarkFactory.SyntaxMark) {
					...
					if (si == null) {
						 ...
						 */
						try {
							doc.op.prepareSyntax(slot, syntax, doc.op.getLeftSyntaxMark(startOffset), startOffset, pos
									- startOffset, true);
						} catch (BadLocationException e) {
