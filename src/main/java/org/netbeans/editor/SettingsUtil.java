...
public class SettingsUtil {
	...
	private static String getColoringSettingName(String coloringName, boolean printingSet) {
		return (coloringName + (printingSet ? SettingsNames.COLORING_NAME_PRINT_SUFFIX
				: SettingsNames.COLORING_NAME_SUFFIX)).intern();
	}
	...
	public static Object getColoring(Class kitClass, String coloringName, boolean printingSet,
	...
	public static Object getTokenColoring(TokenContextPath tcp, TokenCategory tokenIDOrCategory, boolean printingSet,
			boolean evaluateEvaluators) {
	 ...
	 * <tt>SettingsNames.COLORING_NAME_LIST</tt> setting is used to the coloring
	 * names that will apear in the map.
	 * 
	 * @param kitClass
			...
			for (int i = tokenContextList.size() - 1; i >= 0; i--) {
				...
				for (int j = 0; j < allPaths.length; j++) {
					...
					for (int k = 0; k < tokenCategories.length; k++) {
						Object coloring = getTokenColoring(allPaths[j], tokenCategories[k], printingSet,
								evaluateEvaluators);
						if (coloring != null) {
	 ...
	 */
	public static Settings.Initializer getColoringMapInitializer(Class kitClass, Map coloringMap, boolean printingSet,
	...
	public static class PrintColoringEvaluator implements Settings.Evaluator {
		...
		public Object getValue(Class kitClass, String settingName) {
			if (settingName.endsWith(SettingsNames.COLORING_NAME_PRINT_SUFFIX)) {
				String coloringName = settingName.substring(0, settingName.length()
						- SettingsNames.COLORING_NAME_PRINT_SUFFIX.length());
	...
	public static class ForeColorPrintColoringEvaluator extends PrintColoringEvaluator {
		...
		protected Coloring getPrintColoring(Class kitClass, String coloringName, Coloring componentColoring) {
			return Coloring.changeForeColor(super.getPrintColoring(kitClass, coloringName, componentColoring),
					foreColor);
		}
	...
	public static class TokenColoringEvaluator implements Settings.Evaluator, java.io.Serializable {
		 ...
		 *            coloring
		 */
		public TokenColoringEvaluator(TokenContextPath tokenContextPath, TokenCategory tokenIDOrCategory,
				Coloring coloring, boolean printingSet) {
	...
	public static abstract class TokenColoringInitializer extends Settings.AbstractInitializer {
		...
		public void updateSettingsMap(Class kitClass, Map settingsMap) {
			...
			if (kitClass == BaseKit.class) { // all token colorings on base-kit
												// level
		 ...
		 *         coloring.
		 */
		public abstract Object getTokenColoring(TokenContextPath tokenContextPath, TokenCategory tokenIDOrCategory,
				boolean printingSet);
