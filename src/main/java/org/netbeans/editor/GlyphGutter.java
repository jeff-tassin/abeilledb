...
public class GlyphGutter extends JComponent implements Annotations.AnnotationsListener, Accessible {
		...
		while ((y + (lineHeight / 2)) <= (drawHere.y + drawHere.height)) {
			...
			if (showLineNumbers && ((!drawOverLineNumbers) || (drawOverLineNumbers && line != lineWithAnno))) {
				if (line < lineCount) {
					...
					g.drawString("" + (line + 1), numberWidth - lineNumberWidth - rightMargin,
							y + editorUI.getLineAscent());
				}
			}
			...
			if (line == lineWithAnno) {
				...
				if (anno != null) {
					...
					if (!(count == 1 && anno.isDefaultGlyph())) {
						if (prepareImage(anno.getGlyph(), imgObserver))
							g.drawImage(anno.getGlyph(), xPos, y + (lineHeight - anno.getGlyph().getHeight(null)) / 2
									+ 1, null);
					}
				}
				...
				if (count > 1)
					if (prepareImage(gutterButton, imgObserver) && prepareImage(anno.getGlyph(), imgObserver))
						g.drawImage(gutterButton, xPos + glyphWidth, y + (lineHeight - anno.getGlyph().getHeight(null))
								/ 2, null);
	...
	private boolean isMouseOverCycleButton(MouseEvent e) {
		int line = (int) ((float) e.getY() / (float) lineHeight);
		if (e.getX() >= getXPosOfGlyph(line) + glyphWidth
				&& e.getX() <= getXPosOfGlyph(line) + glyphWidth + glyphButtonWidth)
			return true;
		else
			return false;
	}
				...
				if (newSelection) {
					...
					if (dragEndLine < dragStartLine) {
						if (selectForward) {
							...
							if (offSet < 0) {
								offSet = Utilities.getRowEnd(doc,
										Utilities.getRowStartFromLineOffset(doc, dragStartLine));
							}
