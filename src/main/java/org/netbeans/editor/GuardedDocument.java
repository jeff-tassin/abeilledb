...
public class GuardedDocument extends BaseDocument implements StyledDocument {
	...
	protected void preInsertCheck(int offset, String text, AttributeSet a) throws BadLocationException {
		...
		if (text.length() > 0 && (rel & MarkBlock.OVERLAP) != 0 && rel != MarkBlock.INSIDE_END // guarded
			...
			if (!breakGuarded || atomicAsUser) {
				throw new GuardedException(MessageFormat.format(LocaleSupport.getString(FMT_GUARDED_INSERT_LOCALE),
						new Object[] { new Integer(offset) }), offset);
			}
		}
	}
	...
	protected void preRemoveCheck(int offset, int len) throws BadLocationException {
		...
		if ((rel & MarkBlock.OVERLAP) != 0
				|| (rel == MarkBlock.CONTINUE_BEGIN && !(offset == 0 || getChars(offset - 1, 1)[0] == '\n'))) {
			if (!breakGuarded || atomicAsUser) {
				// test whether the previous char before removed text is '\n'
				throw new GuardedException(MessageFormat.format(LocaleSupport.getString(FMT_GUARDED_REMOVE_LOCALE),
						new Object[] { new Integer(offset) }), offset);
			}
		}
	}
