...
public class JCUtilities {
	...
	static {
		int[] typeIDs = new int[] { JavaTokenContext.BOOLEAN_ID, JavaTokenContext.BYTE_ID, JavaTokenContext.CHAR_ID,
				JavaTokenContext.DOUBLE_ID, JavaTokenContext.FLOAT_ID, JavaTokenContext.INT_ID,
				JavaTokenContext.LONG_ID, JavaTokenContext.SHORT_ID, JavaTokenContext.VOID_ID };
		...
		JCClass[][] classesVals = new JCClass[][] {
				...
				new JCClass[] { null, JavaCompletion.BYTE_CLASS, JavaCompletion.INT_CLASS, JavaCompletion.DOUBLE_CLASS,
						JavaCompletion.FLOAT_CLASS, JavaCompletion.INT_CLASS, JavaCompletion.LONG_CLASS,
						JavaCompletion.INT_CLASS, null }, // byte

				new JCClass[] { null, JavaCompletion.INT_CLASS, JavaCompletion.CHAR_CLASS, JavaCompletion.DOUBLE_CLASS,
						JavaCompletion.FLOAT_CLASS, JavaCompletion.INT_CLASS, JavaCompletion.LONG_CLASS,
						JavaCompletion.INT_CLASS, null }, // char

				new JCClass[] { null, JavaCompletion.DOUBLE_CLASS, JavaCompletion.DOUBLE_CLASS,
						JavaCompletion.DOUBLE_CLASS, JavaCompletion.DOUBLE_CLASS, JavaCompletion.DOUBLE_CLASS,
						JavaCompletion.DOUBLE_CLASS, JavaCompletion.DOUBLE_CLASS, null }, // double

				new JCClass[] { null, JavaCompletion.FLOAT_CLASS, JavaCompletion.FLOAT_CLASS,
						JavaCompletion.DOUBLE_CLASS, JavaCompletion.FLOAT_CLASS, JavaCompletion.FLOAT_CLASS,
						JavaCompletion.FLOAT_CLASS, JavaCompletion.FLOAT_CLASS, null }, // float

				new JCClass[] { null, JavaCompletion.INT_CLASS, JavaCompletion.INT_CLASS, JavaCompletion.DOUBLE_CLASS,
						JavaCompletion.FLOAT_CLASS, JavaCompletion.INT_CLASS, JavaCompletion.LONG_CLASS,
						JavaCompletion.INT_CLASS, null }, // int

				new JCClass[] { null, JavaCompletion.LONG_CLASS, JavaCompletion.LONG_CLASS,
						JavaCompletion.DOUBLE_CLASS, JavaCompletion.FLOAT_CLASS, JavaCompletion.LONG_CLASS,
						JavaCompletion.LONG_CLASS, JavaCompletion.LONG_CLASS, null }, // long
	...
	private static boolean typeEqual(JCType t1, JCType t2) {
		return (t1 == null) ? (t2 == null) : classEqual(t1.getClazz(), t2.getClazz())
				&& (t1.getArrayDepth() == t2.getArrayDepth());
	}

	private static boolean parameterEqual(JCParameter p1, JCParameter p2) {
		return (p1 == null) ? (p2 == null) : typeEqual(p1.getType(), p2.getType())
				&& stringEqual(p1.getName(), p2.getName());
	}
	...
	private static boolean constructorEqual(JCConstructor c1, JCConstructor c2) {
		return (c1 == null) ? (c2 == null) : (c1.getClazz().equals(c2.getClazz()) // mustn't
				...
				&& parameterArrayEqual(c1.getParameters(), c2.getParameters()) && classArrayEqual(c1.getExceptions(),
				c2.getExceptions()));
	}
	...
	private static boolean methodArraysEqual(JCMethod[] ma1, JCMethod[] ma2) {
		...
		for (int i = ma1.length - 1; i >= 0; i--) {
			JCMethod m1 = ma1[i];
			JCMethod m2 = ma2[i];
			if (!constructorEqual(m1, m2) || !stringEqual(m1.getName(), m2.getName())
	...
	public static boolean isAssignable(JCType from, JCType to) {
		...
		if (toCls.equals(JavaCompletion.OBJECT_CLASS)) { // everything is object
			return (from.getArrayDepth() > to.getArrayDepth())
					|| (from.getArrayDepth() == to.getArrayDepth() && !JavaCompletion.isPrimitiveClass(fromCls));
		}
	...
	public static JCType getCommonType(JCType typ1, JCType typ2) {
		...
		if (cls1Kwd == null && cls2Kwd == null) { // non-primitive classes
			...
			if (cls1Kwd != null && cls2Kwd != null) {
				return JavaCompletion.getType(primitivesCommonClass[cls1Kwd.getNumericID()][cls2Kwd.getNumericID()],
						typ1.getArrayDepth());
	...
	public static List getClassList(List classNames, boolean storeDeclaredClasses, int classLevel, int fieldLevel,
			int methodLevel) {
	...
	private static List createClassList(Class c, boolean storeDeclaredClasses, int classLevel, int fieldLevel,
			int methodLevel) {
			...
			try {
				...
				for (int i = 0; i < dF.length; i++) {
					if (JavaCompletion.getLevel(dF[i].getModifiers()) >= fieldLevel) {
						lst.add(new JavaCompletion.BaseField(this, dF[i].getName(), createType(dF[i].getType()), dF[i]
								.getModifiers()));
					}
				}
			...
			try {
				...
				for (int i = 0; i < dM.length; i++) {
					if (JavaCompletion.getLevel(dM[i].getModifiers()) >= methodLevel) {
						...
						lst.add(new JavaCompletion.BaseMethod(this, dM[i].getName(), dM[i].getModifiers(),
								createType(dM[i].getReturnType()), parameters, exceptions));
					}
				}
