...
public class JavaSyntaxSupport extends ExtSyntaxSupport {
	...
	private static final TokenID[] COMMENT_TOKENS = new TokenID[] { JavaTokenContext.LINE_COMMENT,
			JavaTokenContext.BLOCK_COMMENT };

	private static final TokenID[] BRACKET_SKIP_TOKENS = new TokenID[] { JavaTokenContext.LINE_COMMENT,
			JavaTokenContext.BLOCK_COMMENT, JavaTokenContext.CHAR_LITERAL, JavaTokenContext.STRING_LITERAL };
				...
				if (dotPos >= 2) { // last char before inserted space
					...
					try {
						...
						if (txtBeforeSpace.endsWith("new ")) {
							if ((txtBeforeSpace.length() > 4)
									&& (!Character
											.isJavaIdentifierPart(txtBeforeSpace.charAt(txtBeforeSpace.length() - 5)))) {
								return ExtSyntaxSupport.COMPLETION_POPUP;
							}
						}

						if (txtBeforeSpace.endsWith("import ")
	...
	public static class JavaDeclarationTokenProcessor implements DeclarationTokenProcessor, VariableMapTokenProcessor {
		...
		public boolean token(TokenID tokenID, TokenContextPath tokenContextPath, int tokenOffset, int tokenLen) {
			...
			case JavaTokenContext.COMMA_ID:
				if (parenthesisCounter > 0) { // comma is declaration separator
												// in parenthesis
