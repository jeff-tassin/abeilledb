...
public class JavaCompletionQuery implements CompletionQuery {
	 ...
	 */
	static List findFieldsAndMethods(JCFinder finder, JCClass cls, String name, boolean exactMatch, boolean staticOnly,
			boolean inspectOuterClasses) {
		// Find inner classes
	...
	static class Context {
		...
		public Context(JCTokenProcessor tp, JTextComponent component, JavaSyntaxSupport sup, boolean sourceHelp,
				int endOffset) {
			this.tp = tp;
		...
		boolean resolveExp(JCExpression exp) {
			...
			case JCExpression.DOT: // Dot expression
				int parmCnt = exp.getParameterCount(); // Number of items in the
														// dot exp
				...
				if (ok && lastDot) { // Found either type or package help
					...
					if (lastType != null) { // Found type
						...
						result = new JavaResult(component, res, formatType(lastType, true, true, true), exp, substPos,
		...
		boolean resolveItem(JCExpression item, boolean first, boolean last) {
					...
					if (first) { // try to find variable for the first item
						if (last && !findType) { // both first and last item
							...
							JCClass cls = sup.getClass(varPos); // get document
																// class
							if (cls != null) {
								res.addAll(findFieldsAndMethods(finder, cls, var, false, sup.isStaticBlock(varPos),
										true));
							}
							...
							if (lastType != null) { // variable found
								...
								lastPkg = finder.getExactPackage(var); // try
																		// package
								if (lastPkg == null) { // not package, let's try
														// class name
							...
							if (findType || !last) {
								...
								if (!inner) { // not inner class name
									if (ad == 0) { // zero array depth
										List fldList = finder.findFields(lastType.getClazz(), var, true, staticOnly,
												false);
								...
								result = new JavaResult(component, findFieldsAndMethods(finder, cls, var, false,
										staticOnly, false), lastType.format(false) + '.' + var + '*', item, cls
										.getName().length() + 1);
							}
			...
			case JCExpression.ARRAY:
				...
				if (cont) {
					...
					if (lastType != null) { // must be type
						if (item.getParameterCount() == 2) { // index in array
																// follows
			...
			case JCExpression.OPERATOR:
				...
				if (curCls != null) { // find all methods and fields for "this"
										// class
					res.addAll(findFieldsAndMethods(finder, curCls, "", false,
			...
			case JCExpression.METHOD: // Closed method
				...
				if (isConstructor) { // Help for the constructor
					...
					if (cls != null) {
						...
						if (mtdList.size() > 0) {
							if (last && !findType) {
								result = new JavaResult(component, mtdList, lastType.getClazz().getFullName() + '.'
										+ mtdName + '(' + parmStr + ')', item, endOffset, 0, 0);
							} else {
								lastType = ((JCMethod) mtdList.get(0)).getReturnType();
								staticOnly = false;
							}
						} else {
							result = new JavaResult(component, ctrList, mtdName + '(' + parmStr + ')', item, endOffset,
									0, 0);
						}
					} else {
						isConstructor = false;
	...
	public static class JavaResult extends CompletionQuery.AbstractResult {
		...
		public JavaResult(JTextComponent component, List data, String title, JCExpression substituteExp,
				int classDisplayOffset) {
			this(component, data, title, substituteExp, substituteExp.getTokenOffset(0), substituteExp
					.getTokenLength(0), classDisplayOffset);
		}

		public JavaResult(JTextComponent component, List data, String title, JCExpression substituteExp,
				int substituteOffset, int substituteLength, int classDisplayOffset) {
			super(data, title);
			...
			try {
				...
				if (commonText != null) {
					if (substituteExp != null) {
						if ((substituteExp.getExpID() == JCExpression.METHOD_OPEN)
								|| (substituteExp.getExpID() == JCExpression.METHOD))
							return true;
					}
					doc.atomicLock();
					try {
		...
		public boolean substituteText(int dataIndex, boolean shift) {
			...
			if (replacement instanceof JCPackage) {
				...
				case JCExpression.METHOD_OPEN:
					...
					if (parms.length == 0) {
						...
						if (f instanceof ExtFormatter) {
							Object o = ((ExtFormatter) f)
									.getSettingValue(JavaSettingsNames.JAVA_FORMAT_SPACE_AFTER_COMMA);
						...
						try {
							if (addSpace
									&& (ind == 0 || (substituteOffset > 0 && Character.isWhitespace(doc.getText(
											substituteOffset - 1, 1).charAt(0))))) {
								addSpace = false;
							}
						} catch (BadLocationException e) {
						}
				...
				default:
					...
					if (f instanceof ExtFormatter) {
						Object o = ((ExtFormatter) f)
								.getSettingValue(JavaSettingsNames.JAVA_FORMAT_SPACE_BEFORE_PARENTHESIS);
						if ((o instanceof Boolean) && ((Boolean) o).booleanValue()) {
							addSpace = true;
						}
					}
			...
			if (text != null) {
				...
				try {
					...
					if (selectionStartOffset >= 0) {
						component
								.select(substituteOffset + selectionStartOffset, substituteOffset + selectionEndOffset);
					}
				} catch (BadLocationException e) {
					// Can't update
				} finally {
