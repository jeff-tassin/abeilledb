...
public class ExtCaret extends BaseCaret {
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (c != null) {
			...
			highlightRow = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.HIGHLIGHT_CARET_ROW,
			...
			highlightBrace = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.HIGHLIGHT_MATCH_BRACE,
			...
			simpleMatchBrace = SettingsUtil.getBoolean(kitClass, ExtSettingsNames.CARET_SIMPLE_MATCH_BRACE,
		...
		if (c != null && highlightBrace) {
			try {
				...
				boolean madeValid = false; // whether brace marks display were
											// validated
				if (dotPos > 0) {
					int[] matchBlk = ((ExtSyntaxSupport) doc.getSyntaxSupport()).findMatchingBlock(dotPos - 1,
							simpleMatchBrace);
					if (matchBlk != null) {
						if (highlightBraceStartMark != null) {
