...
public class ExtSyntaxSupport extends SyntaxSupport {
	 ...
	 *         more tokens in the text, the <tt>Syntax.INVALID</tt> is returned.
	 */
	...
	final class TokenItemTP implements TokenProcessor {
		...
		TokenItemTP() {
			fwdBatchLineCnt = bwdBatchLineCnt = ((Integer) getDocument().getProperty(SettingsNames.LINE_BATCH_SIZE))
					.intValue();
		...
		public boolean token(TokenID tokenID, TokenContextPath tokenContextPath, int tokenBufferOffset, int tokenLength) {
			...
			lastItem = new Item(tokenID, tokenContextPath, bufferStartPos + tokenBufferOffset, new String(buffer,
					tokenBufferOffset, tokenLength), lastItem);
