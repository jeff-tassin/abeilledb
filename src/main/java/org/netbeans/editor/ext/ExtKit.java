...
public class ExtKit extends BaseKit {
	...
	protected Action[] createActions() {
		Action[] extActions = new Action[] { new BuildPopupMenuAction(), new ShowPopupMenuAction(),
				new BuildToolTipAction(), new FindAction(),
				new ReplaceAction(),
				new GotoAction(),
				...
				new EscapeAction(), new ExtDefaultKeyTypedAction(), new ExtInsertTabAction(),
				new ExtInsertBreakAction(), new ExtUpAction(upAction, false), new ExtPageUpAction(pageUpAction, false),
				new ExtDownAction(downAction, false), new ExtPageDownAction(pageDownAction, false),
				new ExtBeginLineAction(beginLineAction, false), new ExtBeginLineAction(selectionBeginLineAction, true),
				new ExtEndLineAction(endLineAction, false), new ExtEndLineAction(selectionEndLineAction, true),
				new CompletionShowAction(), new CompletionHideAction(), new ShiftInsertBreakAction() };
	...
	public static class BuildPopupMenuAction extends BaseAction {
		...
		protected JPopupMenu buildPopupMenu(JTextComponent target) {
			...
			List l = (List) Settings.getValue(Utilities.getKitClass(target),
					(ui == null || ui.hasExtComponent()) ? ExtSettingsNames.POPUP_MENU_ACTION_NAME_LIST
	...
	public static class ToggleCaseIdentifierBeginAction extends BaseAction {
		...
		ToggleCaseIdentifierBeginAction() {
			super(toggleCaseIdentifierBeginAction, ABBREV_RESET | MAGIC_POSITION_RESET | UNDO_MERGE_RESET
					| WORD_MATCH_RESET);
		}
			...
			if (target != null) {
				...
				try {
					if (caret.isSelectionVisible()) {
						...
						doc.insertString(Utilities.getRowStart(doc, target.getSelectionStart()), lineCommentString,
								null); // NOI18N
					}
				} catch (BadLocationException e) {
					target.getToolkit().beep();
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			...
			if ((target != null) && (evt != null)) {
				if ((cmd != null) && (cmd.length() == 1)
						&& ((mod & ActionEvent.ALT_MASK) == 0 && (mod & ActionEvent.CTRL_MASK) == 0)) {
