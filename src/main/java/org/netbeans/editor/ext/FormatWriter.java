...
public final class FormatWriter extends Writer {
	 ...
	 * <tt>BaseDocument</tt>, the syntax is used solely for the scanning of the
	 * formatted tokens and it is first prepared to scan right after the offset.
	 * For non-BaseDocuments the syntax first scans the whole are from the
	 * begining of the document till the offset and then continues on with the
	 * formatted tokens.
	 */
	...
	FormatWriter(ExtFormatter formatter, Document doc, int offset, Writer underWriter, boolean indentOnly) {
		...
		syntax = (kitClass != BaseKit.class) ? BaseKit.getKit(kitClass).createFormatSyntax(doc)
				: new org.netbeans.editor.ext.plain.PlainSyntax();
		...
		if (doc instanceof BaseDocument) {
			try {
				...
				if (offset > 0) { // only if not formatting from the start of
					...
					int startOffset = Utilities.getRowStart(bdoc, Math.max(offset - offsetPreScan, 0),
			...
			try {
				...
				while (tokenID != null) {
					...
					lastToken = new FormatTokenItem(tokenID, syntax.getTokenContextPath(), tokenOffset, text.substring(
							tokenOffset, tokenOffset + syntax.getTokenLength()), lastToken);
	 ...
	 */
	public TokenItem insertToken(TokenItem beforeToken, TokenID tokenID, TokenContextPath tokenContextPath,
			String tokenImage) {
		if (debugModify) {
	 ...
	 */
	public TokenItem splitStart(TokenItem token, int startLength, TokenID newTokenID,
	 ...
	 * <tt>text.length()</tt>. <tt>IllegalArgumentException</tt> is thrown if
	 * offset is wrong.
	 * 
	 * @param token
	...
	public synchronized void write(char[] cbuf, int off, int len, int[] saveOffsets, Position.Bias[] saveBiases)
			throws IOException {
		if (simple) {
			underWriter.write(cbuf, off, len);
			return;
		...
		if (debug) {
			System.err.println("FormatWriter.write(): '" // NOI18N
					+ org.netbeans.editor.EditorDebug.debugChars(cbuf, off, len) + "', length=" + len
					+ ", bufferSize="
					+ bufferSize); // NOI18N
		}
