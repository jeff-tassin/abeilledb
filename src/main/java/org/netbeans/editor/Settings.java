 ...
 * <tt>Settings</tt>. Any class inheritance hierarchy could be used here having
 * the null as the common root.
 * 
...
public class Settings {
	 ...
	 */
	public static synchronized KitAndValue[] getValueHierarchy(Class kitClass, String settingName,
	 ...
	 * until the whole runnable completes. Nesting of <tt>update()</tt> call is
	 * allowed. Only one firing is performed after the whole runnable completes
	...
	private static void fireSettingsChange(Class kitClass, String settingName, Object oldValue, Object newValue) {
		if (firingEnabled) {
			SettingsChangeListener[] listeners = (SettingsChangeListener[]) listenerList
					.getListeners(SettingsChangeListener.class);
	 ...
	 * based on the other setting(s) value. The <tt>Evaluator</tt> instance can
	 * be used as the value in the <tt>Settings.setValue()</tt> call. In that
	 * case the call to the <tt>Settings.getValue()</tt> call will 'evaluate'
	 * the Evaluator by calling its <tt>getValue()</tt>.
	 */
	 ...
	 * <tt>Settings.addFilter()</tt>. Each call to <tt>Settings.getValue()</tt>
	 * will first retrieve the value and then call the
	 * <tt>Filter.filterValue()</tt> to get the final value. Each call to
	 * <tt>Settings.getValueHierarchy()</tt> will first retrieve the
