...
public class BaseKit extends DefaultEditorKit {
	...
	static SettingsChangeListener settingsListener = new SettingsChangeListener() {
		public void settingsChange(SettingsChangeEvent evt) {
			...
			boolean clearActions = (settingName == null || SettingsNames.CUSTOM_ACTION_LIST.equals(settingName) || SettingsNames.MACRO_MAP
					.equals(settingName));
	...
	protected Action[] createActions() {
		return new Action[] { new DefaultKeyTypedAction(), new InsertContentAction(), new InsertBreakAction(),
				new InsertTabAction(), new DeleteCharAction(deletePrevCharAction, false),
				new DeleteCharAction(deleteNextCharAction, true), new ReadOnlyAction(), new WritableAction(),
				new CutAction(), new CopyAction(), new PasteAction(), new BeepAction(), new UpAction(upAction, false),
				...
				new PageDownAction(pageDownAction, false), new ForwardAction(forwardAction, false),
				new ForwardAction(selectionForwardAction, true), new BackwardAction(backwardAction, false),
				...
				new SelectAllAction(), new ActionFactory.RemoveTabAction(), new ActionFactory.RemoveWordAction(),
				...
				new ActionFactory.StartMacroRecordingAction(), new ActionFactory.StopMacroRecordingAction(),
	 ...
	 * <code>getActions()</code> and <code>getActionByName()</code> can be used
	 * safely in this method. The implementation must call
						...
						try {
							...
							if (doInsert) {
								if (caret.isSelectionVisible()) { // valid
									...
									Boolean overwriteMode = (Boolean) editorUI
											.getProperty(EditorUI.OVERWRITE_MODE_PROPERTY);
		...
		public void actionPerformed(ActionEvent evt, JTextComponent target) {
			if (target != null) {
				...
				if (caret.isSelectionVisible()) { // block selected
					try {
						doc.getFormatter().changeBlockIndent(doc, target.getSelectionStart(), target.getSelectionEnd(),
								+1);
	...
	public static class SelectLineAction extends KitCompoundAction {
		...
		public SelectLineAction() {
			super(selectLineAction, new String[] { lineFirstColumnAction, selectionEndLineAction,
