...
public class BaseCaret extends Rectangle implements Caret, FocusListener, MouseListener, MouseMotionListener,
		PropertyChangeListener, DocumentListener, ActionListener, SettingsChangeListener {
	...
	protected void paintCustomCaret(Graphics g) {
		...
		if (c != null) {
			...
			if (THIN_LINE_CARET.equals(type)) { // thin line caret
				...
				if (afterCaretFont.isItalic() && italic) { // paint italic caret
	...
	protected void update(Rectangle scrollRect, int scrollPolicy) {
		...
		if (c != null) {
			...
			if (doc != null) {
				...
				if (italic) { // caret is italic - add char height to the width
								// of the rect
	...
	public void mousePressed(MouseEvent evt) {
		JTextComponent c = component;
		if (c != null) {
			Utilities.getEditorUI(c).getWordMatch().clear(); // [PENDING] should
																// be done
																// cleanly

			// Position the cursor at the appropriate place in the document
			if ((SwingUtilities.isLeftMouseButton(evt) && (evt.getModifiers() & (InputEvent.META_MASK | InputEvent.ALT_MASK)) == 0)
					|| !isSelectionVisible()) {
