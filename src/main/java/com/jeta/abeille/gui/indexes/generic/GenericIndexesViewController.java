 ...
 * @author Jeff Tassin
 */
public class GenericIndexesViewController extends TSController {
	/** the view we are controlling */
	private IndexesView m_view;

	public GenericIndexesViewController(IndexesView view) {
		super(view);
		m_view = view;
	...
	private void editIndex(TableIndex index) {
		...
		GenericIndexesModel model = getModel();
		...
		// final MySQLIndexView iview = new MySQLIndexView(
		// model.getConnection(), model.getTableId(), oldindex );
	 ...
	 */
	GenericIndexesModel getModel() {
		return (GenericIndexesModel) m_view.getModel();
	}
