 ...
 * @author Jeff Tassin
 */
public class ColumnCompletionQuery implements CompletionQuery {
	...
	public ColumnCompletionQuery(SQLCompletion completion, TableSelectorModel model) {
	...
	private SQLCompletionResult buildCompletionResult(JTextComponent component, int offset, SQLTokenProcessor processor) {
		System.out.println("ColumnCompletionQuery.buildCompletionResult");

		SQLToken columntoken = null; // this can be partially completed
		...
		if (tablename != null) {
			try {
				...
				if (tmd != null) {
					result = new ColumnCompletionResult(component, m_completion, tmd, columntoken);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
