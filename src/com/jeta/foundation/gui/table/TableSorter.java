...
public class TableSorter extends TableMap {
	...
	/**
	 * row index for the column in question.
	 */
	int m_indexes[];
	...
	public void checkModel() {
		if (m_indexes.length != model.getRowCount()) {
			if (TSUtils.isDebug()) {
				System.err.println("Sorter not informed of a change in model:  indexes.length = " + m_indexes.length
		...
		model.addTableModelListener(new TableModelListener() {
			public void tableChanged(TableModelEvent e) {
				...
				if (e.getType() == TableModelEvent.INSERT) {
					...
					// System.out.println(
					// "TableSorter got unknown event:  sortmode = " +
					// m_sortmode + "  col = " + m_sortingColumn );
	...
	public void n2sort() {
		...
		for (int i = 0; i < rowcount; i++) {
			for (int j = i + 1; j < rowcount; j++) {
				int result = compare(m_indexes[i], m_indexes[j]);
				if ( result == -1) {
					swap(i, j);
					//System.out.println( "n2sort  i: " + i + "  j: " + j + "  rowcount: " + getRowCount() + " time: " + millsecs );
				}
			}
		}
	}
	 ...
	 * "TableSorter.tableChanged" ); reallocateIndexes(); super.tableChanged(e);
	 * }
	 */
	...
	public void addMouseListenerToHeaderInTable(JTable table, TSTablePanel panel) {
		table.setColumnSelectionAllowed(false);
		JTableHeader th = table.getTableHeader();
		th.addMouseListener(new TableSorterHeaderMouseAdapter(this, table, panel));
	}
}
