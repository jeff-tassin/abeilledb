...
public class Abbrev implements SettingsChangeListener, PropertyChangeListener {
	 ...
	 * string in the text is not accepted by the <tt>addTypedAcceptor</tt>. This
	 * test is only performed if <tt>checkDocText</tt> is true.
	 */
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (kitClass != null) {
			...
			if (settingName == null || SettingsNames.ABBREV_EXPAND_ACCEPTOR.equals(settingName)) {
				doExpandAcceptor = SettingsUtil.getAcceptor(kitClass, SettingsNames.ABBREV_EXPAND_ACCEPTOR,
						AcceptorFactory.FALSE);
			}
			if (settingName == null || SettingsNames.ABBREV_ADD_TYPED_CHAR_ACCEPTOR.equals(settingName)) {
				addTypedAcceptor = SettingsUtil.getAcceptor(kitClass, SettingsNames.ABBREV_ADD_TYPED_CHAR_ACCEPTOR,
						AcceptorFactory.FALSE);
			}
			if (settingName == null || SettingsNames.ABBREV_RESET_ACCEPTOR.equals(settingName)) {
				resetAcceptor = SettingsUtil.getAcceptor(kitClass, SettingsNames.ABBREV_RESET_ACCEPTOR,
						AcceptorFactory.TRUE);
			}
		}
	}
				...
				try {
					...
					if (prevChars.equals(abbrevStr)) { // abbrev chars really
						...
						if (!checkTextDelimiter || dotPos == abbrevStrLen
								|| resetAcceptor.accept(doc.getChars(dotPos - abbrevStrLen - 1, 1)[0])) {
							return abbrevStr;
						}
					}
