...
final class DocMarks {
	...
	int findIndex(Mark mark) {
		...
		int bound = (ind >= gapEnd) ? gapEnd : 0; // boundary for the first pass
		while (true) { // possibly two passes (above and under gap)
			while (ind > bound) {
				if (marks[--ind] == mark) {
					return ind;
	...
	/**
	 * Render marks by some <CODE>Renderer</CODE>. It is the most efficient way
	 * to handle especially multiple adjacent marks. Rendering function is
	 ...
	 *         However the mark can be removed meantime and call
	 *         <CODE>getOffset()</CODE> will throw
	 *         <CODE>InvalidMarkException</CODE>. In that case caller should
	 *         call <CODE>getMarkFromLine()</CODE> again to get another mark and
	 *         retry.
	 */
	...
	private void consistencyError(boolean offsetError, int ind) {
		throw new IllegalStateException("DocMarks.check(): " + (offsetError ? "Offset" : "Line")
				+ " inconsistency found at ind=" + ind + ", mark=" + marks[ind]);
	}
	...
	public String toString() {
		return "marksCount=" + getMarksCount() + ", gapStart=" + gapStart + ", gapEnd=" + gapEnd + ", dataLen="
				+ (Integer.MAX_VALUE - (dataGapEnd - dataGapStart)) + ", dataGapStart=" + dataGapStart
				+ ", dataGapEnd=" + dataGapEnd + ", dataGapLineCount=" + dataGapLineCount;
	}
