...
public class FinderFactory {
	...
	/**
	 * Abstract finder implementation. The only <CODE>find()</CODE> method must
	 * be redefined.
	 */
	 ...
	 * reverse finder for <CODE>PosVisColFwdFinder</CODE>. The starting position
	 * for find should be the start of particular line. The found position will
	 * be that position in document that corresponds to the column position.
	 * This method can be used only in case the font is superfixed i.e. all the
	 * characters of all font styles have the same width.
	 */
			...
			if (stringInd == 0) { // special case for first char
				...
				if (ch == chars[stringInd]) { // matches current char
					...
					if (stringInd == chars.length) { // found whole string
						if (lastChar) {
							found = true;
							return 1 - chars.length; // how many chars back the
														// string starts
						} else {
							wordFound = true;
							return 1;
	...
	public static final class WholeWordsBwdFinder extends GenericBwdFinder implements StringFinder {
		...
		protected int scan(char ch, boolean lastChar) {
			...
			if (stringInd == endInd) { // special case for last char
				if (ch != chars[endInd] || insideWord) { // first char doesn't
															// match
					insideWord = doc.isIdentifierPart(ch);
					return -1;
				} else { // first char matches
