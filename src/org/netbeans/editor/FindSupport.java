...
public class FindSupport {
	...
	public FinderFactory.StringFinder getStringFinder(BaseDocument doc, Map props) {
		FinderFactory.StringFinder stringFinder = (FinderFactory.StringFinder) doc
				.getProperty(BaseDocument.STRING_FINDER_PROP);
	...
	public FinderFactory.StringFinder getStringBwdFinder(BaseDocument doc, Map props) {
		FinderFactory.StringFinder stringBwdFinder = (FinderFactory.StringFinder) doc
				.getProperty(BaseDocument.STRING_BWD_FINDER_PROP);
	...
	public FinderFactory.BlocksFinder getBlocksFinder(BaseDocument doc, Map props) {
		FinderFactory.BlocksFinder blocksFinder = (FinderFactory.BlocksFinder) doc
				.getProperty(BaseDocument.BLOCKS_FINDER_PROP);
	...
	public boolean incSearch(Map props) {
		...
		if (b != null && b.booleanValue()) { // inc search enabled
			...
			if (c != null) {
				...
				DrawLayerFactory.IncSearchLayer incLayer = (DrawLayerFactory.IncSearchLayer) editorUI
						.findLayer(DrawLayerFactory.INC_SEARCH_LAYER_NAME);
				if (incLayer == null) {
	...
	public boolean find(Map props, boolean oppositeDir) {
		...
		if (c != null) {
			...
			try {
				...
				if (blk != null) {
					if (caret instanceof BaseCaret) { // support extended scroll
														// mode
					...
					if (blk[2] == 1) { // wrap was done
						msg += "; "; // NOI18N
						msg += back ? LocaleSupport.getString(WRAP_END_LOCALE) : LocaleSupport
								.getString(WRAP_START_LOCALE);
	 ...
	 */
	public int[] findInBlock(JTextComponent c, int startPos, int blockStartPos, int blockEndPos, Map props,
