	...
	public Action getAction(KeyStroke key) {
		...
		if (ret == null) {
			...
			if (ret != EMPTY_ACTION) { // key that should be ignored
				...
				if (context != null && ret == null) { // no action found when in
														// context
		 ...
		 * ", keyModifiers=" + key.getModifiers() // NOI18N +
		 * ", ignoreNextTyped=" + ignoreNextTyped + ", context=" + context //
		 * NOI18N + ", returning action=" + ((ret == EMPTY_ACTION) ?
		 * "EMPTY_ACTION" : ((ret == null) ? "null" : ((ret instanceof
		 * javax.swing.text.TextAction) // NOI18N ?
