...
public class Analyzer {
	...
	static {
		spacesStrings[0] = "";
		spacesStrings[MAX_CACHED_SPACES_STRING_LENGTH] = new String(getSpacesBuffer(MAX_CACHED_SPACES_STRING_LENGTH),
	...
	public static void initialRead(BaseDocument doc, Reader reader, boolean testLS) throws IOException {
		...
		if (reader != null) {
			...
			int bufStartPos = 0; // Starting position of the buffer in document
			int lineOffset = 0; // Line counter
			int lineLimit = 0; // Longest line found
			int lineStartOffset = 0; // Start offset of the last line
			int nextMarkOffset = markDistance; // Buffer offset of the next mark
												// insertion
					...
					for (offset = 0; offset < readLen; offset++) {
						...
						case '\n':
							if (lastCR) { // ignore this LF
								...
								lastCR = false;
								lineStartOffset++;
								nextMarkOffset++; // correct the removal of the
													// '\n'
	...
	public static String getWhitespaceString(int startCol, int endCol, boolean expandTabs, int tabSize) {
		return (expandTabs || tabSize <= 0) ? getSpacesString(endCol - startCol) : new String(
				createWhiteSpaceFillBuffer(startCol, endCol, tabSize));
	}
