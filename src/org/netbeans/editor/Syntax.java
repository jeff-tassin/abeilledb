 ...
 * <tt>tokenOffset</tt> is the begining of the current token. The <tt>state</tt>
 * variable that identifies the current internal state of the analyzer is set
 * accordingly when the characters are parsed. If the <tt>parseToken()</tt>
 * recognizes a token, it returns its ID and the <tt>tokenOffset</tt> is its
 * begining in the buffer and <tt>offset - tokenOffset</tt> is its length. When
 * the token is processed the value of <tt>tokenOffset</tt> is set to be the
 * same as current value of the <tt>offset</tt> and the parsing continues.
 * 
 * Internal states are the integer constants used internally by analyzer. They
 * are assigned to the <tt>state</tt> variable to express that the analyzer has
 * moved from one state to another. They are usually numbered starting from zero
 * but they don't have to. The only reserved value is -1 which is reserved for
 * the INIT state - the initial internal state of the analyzer.
 * 
 ...
 * in the document. The info is stored in the <tt>StateInfo</tt> interface with
 * the <tt>BaseStateInfo</tt> as the basic implementation. It enables to get and
 * set the two important values from the persistent point of view. The first one
 * is the value of the <tt>state</tt> variable. The other one is the difference
 * <tt>offset - tokenOffset</tt> which is called pre-scan. The particular
 * analyzer can define additional values important for the persistent storage.
 * The <tt>createStateInfo()</tt> can be overriden to create custom state-info
 * and <tt>loadState()</tt> and <tt>storeState()</tt> can be overriden to
 * get/set the additional values.
 * 
...
public class Syntax {
	 ...
	 *            operation. It corresponds to the <tt>stopOffset</tt> that also
	 *            doesn't change through the analyzer operation so any
	...
	public int compareState(StateInfo stateInfo) {
		if (stateInfo != null) {
			return ((stateInfo.getState() == state) && stateInfo.getPreScan() == getPreScan()) ? EQUAL_STATE
					: DIFFERENT_STATE;
		} else {
