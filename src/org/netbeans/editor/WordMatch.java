	...
	public synchronized String getMatchWord(int startPos, boolean forward) {
		...
		if (baseWord == null) {
			try {
				...
				WordInfo info = new WordInfo(baseWordString, startDoc.createPosition(startPos - baseWord.length),
						startDoc);
				...
				while (doc != null) {
					if (doc.getLength() > 0) {
						...
						if (foundPos != -1) { // found
							...
							WordInfo info = new WordInfo(new String(word, 0, wordLen), doc.createPosition(foundPos),
									doc);
							wordsMap.put(info.word, info);
