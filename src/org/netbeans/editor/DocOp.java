...
class DocOp implements AbstractDocument.Content {
	...
	private void update(boolean remove, ModifyUndoEdit undoEdit) {
		...
		try {
			...
			leftUpdateMark = getLeftSyntaxMark(pos - 1); // compute left syntax
															// update mark
	 ...
	 */
	void prepareSyntax(final SyntaxSeg.Slot slot, final Syntax syntax, MarkFactory.SyntaxMark leftSyntaxMark,
			final int reqPos, int reqLen, boolean forceLastBuffer, boolean forceNotLastBuffer)
		...
		if (updatedAreaEnd >= 0 && updatedAreaEnd < reqPos + reqLen) {
			...
			syntax.load(updStateInfo, slot.array, slot.offset + updPreScan, updLen - updPreScan, (reqPos == docLen),
					updMarkPos);
			...
			marks.render(new DocMarks.Renderer() {
				public void render() {
					...
					int index = (updSyntaxMark != null) ? (getNextIndex(getMarkIndex(updSyntaxMark)))
							: getNextIndex(-1);
					int pos = umPos;
					Mark markArray[] = getMarkArray();

					int lastPos = pos; // position of the last syntax mark found
					while (index < markCnt) { // possibly till end of mark array
						...
						if (mark instanceof MarkFactory.SyntaxMark) {
							...
							syntax.relocate(slot.array, slot.offset + lastPos - slotLoadPos, pos - lastPos,
									(pos == docLen), pos);
		...
		syntax.load(stateInfo, slot.array, slot.offset + preScan, prepareLen - preScan, forceNotLastBuffer ? false
				: (reqPos >= docLen), reqPos);

		// go through all the tokens till the required position
		while (syntax.nextToken() != null) {
		}

		syntax.relocate(slot.array, slot.offset + prepareLen, reqLen, forceNotLastBuffer ? false : forceLastBuffer
				|| (reqPos + reqLen >= docLen), reqPos + reqLen);
	}

	void prepareSyntax(final SyntaxSeg.Slot slot, final Syntax syntax, MarkFactory.SyntaxMark leftSyntaxMark,
			final int reqPos, int reqLen, boolean forceLastBuffer) throws BadLocationException {
			...
			try {
				...
				while (index < markCnt) { // possibly till end of mark array
					...
					if (mark instanceof MarkFactory.SyntaxMark) {
						...
						// System.out.println("DocOp.java:600 syntax renderer: scan relocated to buffer='"
