...
public class JCBaseFinder extends JavaCompletion.AbstractProvider implements JCFinder {
	...
	private boolean cheapUpdate(JCClass cls) {
		...
		if (allClassesByName != null && o != null) { // inited and class already
														// there, can do cheap
														// update
	...
	public synchronized List findPackages(String name, boolean exactMatch, boolean subPackages) {
		...
		while (ind < packages.length) {
			...
			if (exactMatch ? (pkgName.length() > nameLen && pkgName.charAt(nameLen) == '.')
	 ...
	 */
	public synchronized List findFields(JCClass cls, String name, boolean exactMatch, boolean staticOnly,
		...
		for (int i = clsList.size() - 1; i >= 0; i--) {
			...
			if (cls != null) {
				...
				List outerList = (i == 0 && inspectOuterClasses && cls.getName().indexOf('.') >= 0) ? getOuterClasses(cls)
						: null;
	 ...
	 */
	public synchronized List findMethods(JCClass cls, String name, boolean exactMatch, boolean staticOnly,
			boolean inspectOuterClasses) {
		TreeSet ts = new TreeSet();
		List clsList = getClassList(cls);
		String pkgName = cls.getPackageName();
