...
public class JCCellRenderer extends JPanel implements ListCellRenderer {
	...
	protected void drawField(Graphics g, JCField fld) {
		...
		drawIcon(g, ((fld.getModifiers() & Modifier.STATIC) == 0) ? icons[FIELD_ICON + level] : icons[FIELD_STATIC_ICON
				+ level]);
	...
	protected void drawMethod(Graphics g, JCMethod mtd) {
		...
		drawIcon(g, ((mtd.getModifiers() & Modifier.STATIC) == 0) ? icons[METHOD_ICON + level]
				: icons[METHOD_STATIC_ICON + level]);
		if (displayStaticWord) {
