...
public class FormatSupport {
	...
	public int getTabSize() {
		...
		return (doc instanceof BaseDocument) ? ((BaseDocument) doc).getTabSize() : formatWriter.getFormatter()
				.getTabSize();
	}

	public int getShiftWidth() {
		Document doc = formatWriter.getDocument();
		return (doc instanceof BaseDocument) ? ((BaseDocument) doc).getShiftWidth() : formatWriter.getFormatter()
				.getShiftWidth();
	}
	...
	public void insertSpaces(TokenItem beforeToken, int spaceCount) {
		...
		insertToken(beforeToken, whitespaceTokenID, null, new String(Analyzer.getSpacesBuffer(spaceCount), 0,
				spaceCount));
	}
	 ...
	 */
	public void replaceToken(TokenItem originalToken, TokenID tokenID, TokenContextPath tokenContextPath,
			String tokenImage) {
	 ...
	 */
	public FormatTokenPosition findNonWhitespace(FormatTokenPosition startPosition, FormatTokenPosition limitPosition,
			boolean stopOnEOL, boolean backward) {
	 ...
	 * <tt>getWhitespaceTokenID()</tt>. Throw <tt>IllegalStateException</tt> if
	 * the whitespace-token-id is null.
	 */
	...
	public FormatTokenPosition changeLineIndent(FormatTokenPosition pos, int indent) {
		...
		if (token == null) { // last line is empty, append the indent string
			if (indentString.length() > 0) {
				token = insertToken(null, getValidWhitespaceTokenID(), getValidWhitespaceTokenContextPath(),
						indentString);
			}
				...
				if (indentString.charAt(indentStringInd) != text.charAt(offset)) {
					if (canModifyWhitespace(token)) {
						...
						if (isWhitespace(token, offset) || offset > 0) {
							...
							insertToken(token, getValidWhitespaceTokenID(), getValidWhitespaceTokenContextPath(),
