...
public class ScrollCompletionPane extends JScrollPane implements CompletionPane, PropertyChangeListener,
	...
	public void settingsChange(SettingsChangeEvent evt) {
		...
		if (kitClass != null) {
			minSize = (Dimension) SettingsUtil.getValue(kitClass, ExtSettingsNames.COMPLETION_PANE_MIN_SIZE,
					ExtSettingsDefaults.defaultCompletionPaneMinSize);
			maxSize = (Dimension) SettingsUtil.getValue(kitClass, ExtSettingsNames.COMPLETION_PANE_MAX_SIZE,
		...
		if (ExtEditorUI.COMPONENT_PROPERTY.equals(propName)) {
			if (evt.getNewValue() != null) { // just installed
				...
				if (view != null) {
					...
					view.registerKeyboardAction(kit.getActionByName(ExtKit.completionHideAction),
	...
	protected Rectangle getPreferredBounds() {
		...
		if (view != null) {
			...
			correction.y = (component.getParent().getLocationOnScreen().y - component.getRootPane()
					.getLocationOnScreen().y) - 1;
			correction.x = (component.getParent().getLocationOnScreen().x - component.getRootPane()
					.getLocationOnScreen().x) - 1;
