...
public class HTMLCompletionQuery implements CompletionQuery {
	...
	public CompletionQuery.Result query(JTextComponent component, int offset, SyntaxSupport support) {
		...
		try {
			...
			/*
			 * if( prev == null && !inside ) System.err.println(
			 * "Beginning of document, first token = " + item.getTokenID() );
			 * else if( item == null ) System.err.println(
			 * "End of document, last token = " + prev.getTokenID() ); else if(
			 * ! inside ) System.err.println( "Between tokens " +
			 * prev.getTokenID() + " and " + item.getTokenID() ); else
			 * System.err.println( "Inside token " + item.getTokenID() );
			 */
			...
			if ((id == HTMLTokenContext.TEXT || id == HTMLTokenContext.VALUE) && preText.endsWith("&")) {
				...
				if (elem.getType() == SyntaxElement.TYPE_TAG) { // not endTags
					...
					List possible = tag.getAttributeList(prefix); // All attribs
																	// of given
																	// tag
			...
			if (l == null)
				...
				return new CompletionQuery.DefaultResult(component, "Results for DOCTYPE " + dtd.getIdentifier(), l,
						offset, len);
	...
	private static abstract class HTMLResultItem implements CompletionQuery.ResultItem {
		...
		/**
		 * @return Properly colored JLabel with text gotten from
		 *         <CODE>getPaintText()</CODE>.
		 */
		...
		/**
		 * The string used in painting by <CODE>getPaintComponent()</CODE>. It
		 * defaults to delegate to <CODE>getItemText()</CODE>.
		 * 
