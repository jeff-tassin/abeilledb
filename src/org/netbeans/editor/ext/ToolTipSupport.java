...
public class ToolTipSupport extends MouseAdapter implements MouseMotionListener, ActionListener,
		PropertyChangeListener, SettingsChangeListener, FocusListener {
	 ...
	 *            status is changed to {@link #STATUS_VISIBILITY_ENABLED} and @link
	 *            #updateToolTip()} is called.<BR>
	 *            It is still possible that the tooltip will not be showing on
	 *            the screen in case the tooltip or tooltip text are left
	 *            unchanged.
	 */
	...
	/**
	 * @return Whether the tooltip is showing on the screen. {@link #getStatus() }
	 *         gives the exact visibility state.
	 */
	...
	/**
	 * @return status of the tooltip visibility. It can be {@link #STATUS_HIDDEN}
	 *         or {@link #STATUS_VISIBILITY_ENABLED} or
	...
	public void setEnabled(boolean enabled) {
		if (enabled != this.enabled) {
			...
			firePropertyChange(PROP_ENABLED, enabled ? Boolean.FALSE : Boolean.TRUE, enabled ? Boolean.TRUE
					: Boolean.FALSE);
	...
	/**
	 * @return last mouse event captured by this support. This method can be used
	 *         by the action that evaluates the tooltip.
	 */
